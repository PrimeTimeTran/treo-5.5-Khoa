...
const HomePage = () => {
  ...
  const fetchMovies = useCallback(async (newM) => {
  ...
  const getCategory = useCallback((newG) => {
      ...
      <div className="main-content">
        ...
        <div className="movies-content">
          ...
          <div className="sort">
            <button
              onClick={() =>
                fetchMovies(movies.sort((a, b) => a.popularity - b.popularity))
              }
              className="sort-btn"
            >
            ...
            <button
              onClick={() =>
                fetchMovies(movies.sort((a, b) => b.popularity - a.popularity))
              }
              className="sort-btn"
            >
          ...
          <Row>
            <Col className="col-card">
              <ModalBox query={"Mortal Combat"} />

              {movies.map((m) => {
                return (
                  <Card
                    key={m.id}
                    style={{ width: "20rem", margin: "20px", height: "860px" }}
                  >
                    <Card.Img
                      variant="top"
                    ...
                    <Card.Body>
                      ...
                      <div className="genres">
                        {genres.genres
                          .filter((g) => m.genre_ids.includes(g.id))
                          .map((e) => (
                            <button className="genre-name-btn" key={e.id}>
                              {e.name}
                            </button>
                          ))}
                      </div>
                      ...
                      <div className="rating">
                        ...
                        <h4>
                          <strong>{m.vote_average} </strong>
                        </h4>{" "}
                        <h6> from </h6> <h6> {m.vote_count} </h6>{" "}
                        <h6> votes </h6>
                      </div>
